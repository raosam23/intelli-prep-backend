...
from app.services.resume_service import upload_resume, get_resumes, get_resume_by_id, delete_resume, update_resume
...
async def update_a_resume_by_id(resume_id: uuid.UUID, new_file: UploadFile, current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)) -> ResumeResponse:
    return await update_resume(session, current_user.id, resume_id, new_file)